...
@Component({
  selector: 'app-insertdeclaracionbimestral',
  standalone: true,
  imports: [ReactiveFormsModule,CommonModule],
  templateUrl: './insertdeclaracionbimestral.component.html',
  styleUrl:    './insertdeclaracionbimestral.component.scss'
})
export class InsertdeclaracionBimestralComponent  {
  ...
  constructor(private route: ActivatedRoute, private fb: FormBuilder, private http: HttpClient, private router: Router, private declaracionBimestralService: DeclaracionBimestralService) {
  ...
  loadUserData(delaracionAnualId: number): void {
    this.declaracionBimestralService.getDeclaraacionBimestralById(delaracionAnualId).subscribe(declaracion => {
  ...
  onSubmit(): void {
    ...
    if (this.declaracioanualForm.valid) {
      ...
      if (this.isEditMode) {
        ...
        this.declaracionBimestralService.updategetDeclaraacionBimestral(delaracion).subscribe({
          next: () => {
            this.isLoading = false;
            alert('declaración bimestarl modificado correctamente');
            this.router.navigate(['/declaracionbimestrallista']);
          },
          error: (err) => {
            this.isLoading = false;
            this.errorMessage = 'Hubo un error al modificar el declaración bimestral';
            console.error(err);
          }
        });
      } else {
     ...
     this.declaracionBimestralService.adddeclaracionbimestral(delaracion).subscribe({
        next: () => {
          this.isLoading = false;
          alert('declaración bimestral insertado correctamente');
          this.router.navigate(['/declaracionbimestrallista']);
        },
        error: (err) => {
          this.isLoading = false;
